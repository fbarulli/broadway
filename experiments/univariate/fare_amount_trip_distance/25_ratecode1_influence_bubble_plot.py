"""25: influence plot — Cook's distance as circular bubbles (LR influence plot).
...
def main() -> None:
    ...
    r_int = infl.resid_studentized_internal
    cooks, _ = infl.cooks_distance

    n, k = len(df), int(model.params.size)
    # Externally studentized residuals via closed form. statsmodels' own
    # property refits the model once per row (leave-one-out loop) — minutes
    # at n=42,806; this exact equivalent is vectorized:
    # t_ext^2 = t_int^2 * (n - k - 1) / (n - k - t_int^2)
    r = r_int * np.sqrt((n - k - 1) / (n - k - r_int**2))
