"""Combined root experiment scripts.
...
- ``verify``     — lightweight verification of the experiments/ tree and the
                   experiment config YAMLs (formerly verify_experiments.py).

Usage:

    uv run python experiments.py <command>
    uv run python -m experiments <command>
"""
...
def main() -> int:
    ...
    p = sub.add_parser("verify", help="lightweight verification of the experiments/ tree + configs")
