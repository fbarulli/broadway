"""Controlled lifecycle for the branch-tracked STATE record."""
...
COLUMNS = ("id", "kind", "status", "owner", "custody", "updated", "source", "summary")
...
def _record_from_args(args: argparse.Namespace) -> Record:
    ...
    values.update(status=_status(values["status"]), updated=_now())
...
def apply_record_operation(args: argparse.Namespace, operation: Literal["add", "update", "close", "void"]) -> None:
    """Only STATE mutation boundary: commit intent, then archive terminal rows."""
    _validate_repo_layout()
    with _state_lock():
        ...
        if operation == "add":
            ...
            if operation == "update":
                ...
                values.update(status=terminal.disposition.lower(), summary=f"{terminal.disposition}: {args.reason}; {values['summary']}", updated=_now())
                record = Record(values); planned = _replace(snapshot.records, index, record)
        if getattr(args, "dry_run", False):
            print("STATE DRY-RUN: no file change was made; planned row:")
...
def parser() -> argparse.ArgumentParser:
    ...
    for name, command_function in (("snapshot", command_snapshot), ("check", command_check)):
