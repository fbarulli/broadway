"""Validate DataFrame against DatasetContract — columns, dtypes, nulls."""
...
def run(cfg: PipelineConfig) -> None:
    ...
    df = load(cfg.dataset)
    issues = (
        check_columns(df, cfg.dataset)
        + check_dtypes(df, cfg.dataset)
        + check_nulls(df, cfg.dataset, cfg.contracts.null_threshold)
    )
    if issues:
