...
def train(
    ...
    **params: float | str,
    ...
    """Build the experiment's Pipeline, fit it, and record the result."""
    model = build_model_pipeline(cfg, cfg.experiment.model.type, params)  # type: ignore[union-attr]
    start = time.time()
    model.fit(X_train, y_train)
    elapsed = time.time() - start
    return model, TrainingResult(
        model_type=cfg.experiment.model.type,  # type: ignore[union-attr]
        params=params,
