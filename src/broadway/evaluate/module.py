...
def _load_val_features(cfg: PipelineConfig) -> tuple[pd.DataFrame, pd.Series]:
    ...
    return val_df[eligible_feature_columns(val_df, cfg)], val_df[cfg.dataset.target]
...
def _load_train_features(cfg: PipelineConfig) -> tuple[pd.DataFrame, pd.Series]:
    ...
    return train_df[eligible_feature_columns(train_df, cfg)], train_df[cfg.dataset.target]
