...
def _plot_chunk(
    ...
    ax_flat = axes.ravel()
    for ax, (name, osm, osr) in zip(ax_flat, traces):
        ax.scatter(osm, osr, s=6, alpha=0.55, edgecolor="none")
...
def _plot_dist_chunk(
    ...
    ax_flat = axes.ravel()
    for ax, (name, counts, edges) in zip(ax_flat, hists):
...
def plot_numeric_qq(
    ...
    *,
    ...
    for name in _numeric_cols(df, cols):
        ...
        features.append(QqFeature(
            feature=name, n_valid=n_valid, n_excluded=n_excluded,
            mean=float(finite.mean()), std=std_val, status="plotted",
            reason=None, figure="",
        ))
    ...
    figures: list[str] = []
    dist_figures: list[str] = []
    for fig_num, chunk in enumerate(chunks, start=1):
        ...
        _plot_chunk(
            ...
            fig_num, len(chunks), fig_size_per_subplot, dpi,
        )
        _plot_dist_chunk(
        ...
        for n in chunk:
            i = name_to_idx[n]
            features[i] = features[i].model_copy(
                update={"figure": fig_name, "dist_figure": dist_name}
            )
