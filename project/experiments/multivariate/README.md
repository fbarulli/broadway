...
All analysis policy lives in `project/config/experiments/multivariate.yaml` (no
...
(`project/config/experiments/working.yaml`, single source of truth); the zone lookup
...
interpretation (`model_verdicts` in `project/config/experiments/multivariate.yaml`).

## Run order

```
uv run python project/experiments/multivariate/04_baseline_sample.py
uv run python project/experiments/multivariate/05_lightgbm_baselines.py
uv run python project/experiments/multivariate/06_model_verdict.py
```
