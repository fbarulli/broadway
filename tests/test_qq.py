...
def test_plot_numeric_qq_excludes_and_splits(tmp_path) -> None:
    ...
    overview = plot_numeric_qq(df, figures_dir, evidence_path)
...
def test_render_profile_includes_qq_evidence() -> None:
    qq = QqOverview(
        source_path="artifacts/discover/qq_overview.json",
        total_features=3,
        excluded_features=1,
        excluded_notes=["const: zero variance"],
        features=[
            QqFeature(feature="a", n_valid=4, n_excluded=0, mean=2.5, std=1.0, figure="figures/numeric_qq_1.png"),
            QqFeature(feature="b", n_valid=4, n_excluded=0, mean=2.5, std=1.0, figure="figures/numeric_qq_1.png"),
        ],
