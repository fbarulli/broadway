...
# appended ids 100…153 continue the GLOBAL dense order prefixed by the owning
...
# no rows; future mints append ≥154.
BAND_RANGES = {
    "INGEST": [(1, 9)],
    "ETL": [(10, 19), (115, 117)],
    "FEAT": [(20, 29)],
...
def test_query_findings_sheet_matches_meta_counts(capsys):
    ...
    assert len(body) == REG["meta"]["counts"]["gates_with_findings"] == 85
    assert footer == (f"== 85 flagged gate(s) · "
