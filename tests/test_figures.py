    ...
    df = pd.DataFrame(
        {
            ...
            "price": [10.0, 12.0, 11.0, 20.0, 22.0, 21.0, 30.0, 28.0, 29.0],
        }
    )
    figures_dir = tmp_path / "reports" / "figures"
    step = runners.run_describe(
        cfg.analysis, 1, "q?", df, "neighborhood", "neighborhood",
        ["A", "B", "C"], "price", "x.parquet",
        None, "canonical", tmp_path / "timeline" / "test_hypothesis", figures_dir,
    )
...
def test_run_normality_sets_figures(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cfg = _cfg()
    groups = {
        "A": np.random.default_rng(0).normal(10.0, 2.0, 30),
        "B": np.random.default_rng(1).normal(12.0, 2.0, 30),
    }
    figures_dir = tmp_path / "reports" / "figures"
    step = runners.run_normality(
        cfg.analysis, 2, "q?", groups,
        tmp_path / "timeline" / "test_hypothesis", figures_dir, "canonical", None,
    )
...
def test_render_timeline_figures_relative_to_reports() -> None:
    ...
    md = render_timeline("test", seq, [_figure_step()], [])
...
def test_render_results_figures_relative_to_results() -> None:
    seq = load_walkthrough_sequence()
    pages = render_results("test", seq, [_figure_step()], [])
...
def test_write_results_figures_relative_to_results(
    ...
    seq = load_walkthrough_sequence()
    write_results("test", seq, [_figure_step()], [])
